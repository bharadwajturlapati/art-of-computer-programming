...
public class DatagramServer extends JFrame {
	/**
	 * Serialization Identifier. Can be volatile/transient
	 */
	...
	public DatagramServer() {
		...
		try (DatagramSocket socket = new DatagramSocket(8000)) {
			...
			while (true) {
				...
				// Compute area
				String requestedString = new String(buf).trim();
				String responseString  = requestedString+" // " + requestedString.length() +" characters.";
				jta.append(responseString);
				...
				sendPacket.setData(responseString.getBytes());
